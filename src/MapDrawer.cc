...
namespace ORB_SLAM2
{
    ...
    void MapDrawer::DrawMapLines()
    {
        ...
        for (size_t i = 0, iend = vpMLs.size(); i < iend; i++)
        {
            ...
            Vector3d sp_eigen = pos.head(3);
            Vector3d ep_eigen = pos.tail(3);
    ...
    void MapDrawer::DrawManhAxis()
    {
        const cv::Mat vpWManhAxis = mpMap->GetWorldManhAxis();

        if (vpWManhAxis.empty())
            return;

        float axis_size = 0.25;

        cv::Mat manh_axis = vpWManhAxis;
