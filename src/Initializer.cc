...
Initializer::Initializer(const Frame &ReferenceFrame, float sigma, int iterations)
{
    ...
    mvKeyLines1 = ReferenceFrame.mvKeylinesUn; 
    mvKeyLineFunctions1 = ReferenceFrame.mvKeyLineFunctions;   
                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    // ref: http://en.cppreference.com/w/cpp/utility/functional/ref
...
void Initializer::ReconstructLine(vector<Match> &vLineMatches, cv::Mat &K, cv::Mat &R21, cv::Mat &t21, vector<Point3f> &vLineS3D, vector<Point3f> &vLineE3D, vector<bool> &vbLineTriangulated)
{
    ...
    for(size_t i=0, iend = vLineMatches.size(); i<iend; i++)
    {
        ...
        cv::Mat L3dSC1; 
        cv::Mat L3dEC1; 
        Vector3d &klf1 = mvKeyLineFunctions1[vLineMatches[i].first];    
        Vector3d &klf2 = mvKeyLineFunctions2[vLineMatches[i].second];   
